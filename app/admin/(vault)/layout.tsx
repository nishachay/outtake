...
export default async function AdminVaultLayout({
  ...
  return (
    <div className="flex min-h-screen">
      <aside className="sticky top-0 hidden h-screen w-60 shrink-0 flex-col gap-1 border-r border-line p-4 md:flex">
        <Link href="/admin" className="admin-h mb-4 flex items-center gap-2 text-lg font-extrabold">
          <Disc3 size={18} />
          Admin
        </Link>
        {NAV.map(({ href, label, icon: Icon }) => (
          <Link
            key={href}
            href={href}
            className="flex items-center gap-3 rounded-full px-3 py-2 text-sm font-semibold text-mut transition hover:bg-[var(--pill-bg)] hover:text-fg"
          >
            <Icon size={16} /> {label}
          </Link>
        ))}
